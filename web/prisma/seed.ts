...
async function main() {
  ...
  // ==================== Organization & Policy ====================
  console.log("🏢 Creating default Organization & Branch...");

  const org = await prisma.organization.create({
    data: {
      name: "Pawnify Demo Organization",
      slug: "pawnify-demo",
  ...
  const branch = await prisma.branch.create({
    data: {
      ...
      name: "Main Branch",
      address: "100 Financial Street, Core District",
    },
  });
  ...
  const loanPolicy = await prisma.loanPolicy.create({
    data: {
      ...
      gracePeriodDays: 7,
      mandatoryIdThreshold: new Decimal("50000"),
      ltvTiers: [
  ...
  const panDocType = await prisma.documentType.create({
    data: {
      organizationId: org.id,
      name: "PAN Card",
      isPrimaryId: true,
  ...
  await prisma.documentType.createMany({
    data: [
      { organizationId: org.id, name: "Passport", isPrimaryId: true },
      { organizationId: org.id, name: "Driver's License", isPrimaryId: false },
      { organizationId: org.id, name: "Aadhaar Card", isPrimaryId: false, maskingRule: "last4" },
    ],
  });
  ...
  await prisma.user.update({
    where: { id: adminId },
    data: { role: "ADMIN", emailVerified: true, isActive: true, organizationId: org.id, branchId: branch.id },
  });
  ...
  await prisma.user.update({
    where: { id: staff1Id },
    data: { emailVerified: true, isActive: true, organizationId: org.id, branchId: branch.id },
  });
