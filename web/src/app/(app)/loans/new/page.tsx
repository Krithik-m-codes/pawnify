...
import { useLazySearchCustomersQuery } from "@/lib/redux/api/customersApi";
...
function NewLoanForm() {
  ...
  const [triggerSearchCustomer] = useLazySearchCustomersQuery();
  ...
  useEffect(() => {
    if (initialCustomerId && !selectedCustomer) {
      triggerSearchCustomer(initialCustomerId)
        .unwrap()
        .then((data) => {
  ...
  }, [initialCustomerId, selectedCustomer, triggerSearchCustomer]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setSearching(true);
      triggerSearchCustomer(customerQuery)
        .unwrap()
        .then((data) => {
  ...
  }, [customerQuery, triggerSearchCustomer]);
  ...
  const { data: marketRatesData } = useGetMarketRatesQuery();

  useEffect(() => {
    if (marketRatesData) {
      const gold = marketRatesData.goldRatePerGram;
      const silver = marketRatesData.silverRatePerGram;
      const margin = marketRatesData.safetyMarginPercent || 0;
      ...
      setSpotRates({
        ...
        silver: effSilver,
        margin,
        ltvTier1Percent: marketRatesData.ltvTier1Percent || 85,
        ltvTier2Percent: marketRatesData.ltvTier2Percent || 80,
        ltvTier3Percent: marketRatesData.ltvTier3Percent || 75,
        ltvTier1Max: marketRatesData.ltvTier1Max || 250000,
        ltvTier2Max: marketRatesData.ltvTier2Max || 500000,
      });

      if (marketRatesData.defaultInterestMonthly) {
        setInterestRateMonthly(marketRatesData.defaultInterestMonthly);
      }
      if (marketRatesData.defaultGraceDays) {
        setGracePeriodDays(marketRatesData.defaultGraceDays);
      }
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-(--text-secondary) font-mono">
                    {selectedCustomer.phonePrimary} • KYC: {selectedCustomer.kycStatus}
                  </div>
                </div>
              </div>
              <button
                    ...
                    >
                      <div>
                        ...
                        <div className="text-[11px] text-(--text-muted) font-mono">
                          {c.phonePrimary} • KYC: {c.kycStatus}
                        </div>
                      </div>
